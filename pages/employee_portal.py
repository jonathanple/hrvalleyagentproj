...
def get_openai_client():
    """Get an OpenAI client using environment variable."""
...
def find_semantic_matches(question, pdf_content):
    ...
    Please identify the 3-5 most relevant sections from this HR document that contain information and give a concise answer the question.
...
"""
        ...
        response = client.chat.completions.create(
            ...
            temperature=0.7,
            max_tokens=1000,
            timeout=40  # allow a long wait if needed
        )
        ...
        import traceback
        print("🚨 Assistant crashed. Here's the traceback:")
        traceback.print_exc()

        return {
            "answer": f"Hi 👋\n\nI'm really sorry — something went wrong while I was trying to help you.\n\n```{str(e)}```\n\nPlease try again or contact HR.",
            "suggestions": [],
            "topic": "Error"
        }
...
def main():
    ...
    if (
        ...
        and len(st.session_state.suggestions) >= 1
    ):
    ...
    if user_input:
        ...
        with st.spinner("Thinking..."):
            ...
            if not response_data:
                st.error("❌ Failed to generate assistant response.")
                return
